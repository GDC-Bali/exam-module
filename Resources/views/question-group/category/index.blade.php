...
@section('script_exam')
    ...
    $(document).ready(function(){        
        var table = $('#example').DataTable({            
            ...
            serverSide: true,
            ajax: {
                url: '{{ route("exam.group-category.getdata") }}'
            },  
        ...
        $('#example tbody').on( 'click', 'tr td:not(:first-child)', function () {                  
            window.location.href = "{{route('exam.group-category.index')}}/"+table.row($(this).parent()).data().id+"/edit/";
        });        
        ...
        $('#delete').on( 'click', function () {
            ...
            confirmButtonText: 'Yes, delete it!'
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        url: "{{route('exam.group-category.multipledelete')}}",
                        type: 'POST',
                        data: {
                            "id": del,
                            "_token": "{{csrf_token()}}",
